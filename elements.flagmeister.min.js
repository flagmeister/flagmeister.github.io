!function () {
    ...
    }; Object.keys(L).map(e => { let f = (f, t) => customElements.define("flag-" + e, class extends f { get data() { return L[e] } static get observedAttributes() { return ["source", "box", "draw", "clip", "filter", "char", "selected", "is"] } attributeChangedCallback(e, f, t) { this.img && f !== t && this.load() } constructor(f) { f = super(), [["iso", e], ["detail", !1], ["source", !1], ["box", "0 0 640 480"], ["transform", ""], ["clip", i(0, 0, 640, 480)], ["flags", L], ["draw", ""], ["filter", !1]].map(([e, t]) => { Object.defineProperty(f, e, { set(t) { t ? f.setAttribute(e, t) : f.removeAttribute(e) }, get() { let r = f.getAttribute(e) || getComputedStyle(this).getPropertyValue("--flagmeister" + e).trim() || t; return "none" == r ? r = !1 : "string" == typeof r && (r = w(r, this)), r && 1 === r.length ? r[0] : r } }) }) } svg(f = e, t = w(L[f], this), r = this.filter, c = this.clip) { return `<svg xmlns='http://www.w3.org/2000/svg' viewBox='${this.box}'><defs><clipPath id='clip'>${"0" === c ? "" : c}</clipPath></defs>` + (r || "") + `<g ${r ? "filter='url(#ff)'" : ""}><g clip-path='url(#clip)' transform='${this.transform}'>${t}${this.draw}</g></g></svg>` } connectedCallback() { let t = this; t.setAttribute("is", "flag-" + e), t.img = t, f == HTMLElement && (t.img = t.querySelector("img") || t.appendChild(document.createElement("img")), t.img.onload = t.onload, t.img.onerror = t.onerror), !t.detail && L[e].includes("detail:") && (t.detail = L[e].split`detail:`[1].split`;`[0]), t.load() } reset() { this.removeAttribute("detail"), this.connectedCallback() } load(f = this.svg()) { let t, r = async f => { try { this.detail = f; let t = {}; location.href.includes("no-cors") && (t = { mode: "no-cors" }); let l = await fetch(f, t), a = await ($(l.url) ? l.text() : l.json()); "string" == typeof a ? c(a) : a[0].flags ? (this.info = a[0], r(a[0].flags.svg)) : console.error("No flag", e, a) } catch (e) { console.error(e, this) } }, c = (f, c = this) => { c.img.src = "data:image/svg+xml," + f.replace(/#/g, "%23"), c.O = new ResizeObserver(f => { c.detail && !$(c.detail) && f[0].contentRect.width >= c.detail && (c.O.disconnect(c), c.detail = ".svg", (t = c.source) ? ($(t) || (t = t + e + ".svg"), c.load(t)) : r("https://restcountries.com/v3.1/alpha/" + e)) }), c.detail && "9999" != c.detail && c.O.observe(c.img) }; $(f) ? r(f) : c(f) } }, t); return f(HTMLElement, {}) }); customElements.define("flagmeister-text", class extends HTMLElement { connectedCallback(e = this, f = e.getAttribute("word"), t = e.getAttribute("iso").split`,`, r = e.getAttribute("height") || "var(--flagmeisterletterheight,99px)") { e.innerHTML = `<style>[word='${f}']{display:flex;justify-content:flex-start}[word='${f}'] img{width:auto;height:${r};filter:var(--flagmeistertextfilter,drop-shadow(1px 1px 0 grey) drop-shadow(-1px -1px 0 #fff) drop-shadow(4px 4px 2px #000))</style>` + [...f].map((r, c) => `<flag-${t[c] || t[0]} detail=9999 letter=${r} clip='text:${JSON.stringify({ str: "_" == r ? " " : r, y: f.match(/[gjqyQ]/) ? 355 : 420, ...JSON.parse(e.getAttribute("text")) })}' filter=light box='150 0 340 480'></flag-${t[c] || t[0]}>`).join`` } })
}();